...
if does_drive_exist("H"):
    directory = "H://Classes/"
    print("Drive H: exists")
else:
    directory = "/Classes/"
    print("Drive H: does not exist")
def setup(): # This is the class creator / setup
    classname = input("What is the name of the class you want to set up? ")
    os.mkdir(directory + classname) # Makes the folder for the class | If you see H:// its cause of school computers
    ...
    f = open(directory + classname + "/people.txt", "w") # Creates the file for the list of students, and opens it to be written.
...
def classcodes(classname):  # creates the class code and returns with the b64 string
    f = open(directory + classname + "/people.txt", "r")
...
def readfromandsep(textfile, seperator): # reads from a file than separates it by the seperator (BY ME)
    f = open(textfile, "r")
...
def attendance():
    pog = readfromandsep(directory + classname + "/people.txt", ",") # Takes the file and seperates the "NAME,NAME,NAME" format.
    ...
    r = open(directory + classname + "/attendance" + str(date.today()) + ".txt", "w") # Creates file and writes too it
    ...
    print("Saved attendance for the day to " + directory + classname + "/attendance" + str(date.today()) + ".txt") # Just tells you a quick link to your files
    exit(1) # program is done, nothing more after this point


if os.path.exists(directory) == False: # If my classes file doesnt exist than make it
    os.mkdir(directory)
...
if any(gaming.lower() == f for f in ["yes", 'y', '1', 'ye']):
    ...
    if not pogaming[0] in os.listdir(directory):
        os.mkdir(directory + pogaming[0]) # Make the class file if it doesnt exist, they just allows you to overwrite classes
    f = open(directory + pogaming[0] + "/people.txt", "w")
...
dir = os.listdir(directory) # had to move this because after inputing a classcode it still thought the directory was empty
